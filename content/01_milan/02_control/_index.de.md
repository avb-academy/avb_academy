...
Milan AVB enthält eine Spezifikation, wie Geräte konfiguriert und {{< termbase "monitor">}} werden können. Diese ist im *AVB/TSN Discovery, Enumeration, Connection management, and Control* Protocol (ATDECC) festgelegt.
...
- **{{< termbase "Device Discovery">}}**: Mechanismen, mit denen Geräte im Netzwerk erkannt werden, sodass sie zuverlässig identifiziert und enumeriert werden können.  
- **{{< termbase "Connection Management">}}**: Aufbau, Verwaltung und Abbau von Verbindungen zwischen Geräten im Netzwerk.  
- **{{< termbase "Control Protocol">}}**: Legt fest, wie Geräte {{< termbase "control">}} und konfiguriert werden. Dazu gehören die Verwaltung von Geräteeinstellungen und Konfigurationen sowie die Gewährleistung eines ordnungsgemäßen Betriebs innerhalb des Netzwerks.

<!-- ## Controllers -->
## {{< termbase "Controller">}}
<!-- There are currently a couple of solutions available: -->
Derzeit gibt es einige Lösungen auf dem Markt:
